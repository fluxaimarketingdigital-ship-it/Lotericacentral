...
function ARels({cl,setCl,pr,setPr,ops,opPrizes,setOpPrizes,cfg,setCfg,campanhas,setCampanhas,adminLogs,setAdminLogs,checkM,adminSel}){
  ...
  const excluirCampanha = async (camp) => {
    ...
    alert("✅ Campanha removida do histórico.");
  };
        ...
        {campanhas.filter(c => !histFilter || (c.nome||"").toLowerCase().includes(histFilter.toLowerCase()) || fD(c.inicio).includes(histFilter) || fDT(c.dataFechamento).includes(histFilter)).map(camp => (
          <div key={camp.id} style={{background:"#fff",borderRadius:14,padding:14,border:`1px solid ${C.bd}`}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:10}}>
              ...
              {isM && (
                <button onClick={()=>excluirCampanha(camp)} style={{background:C.rdC, color:C.rd, border:"none", borderRadius:8, padding:"5px 10px", fontSize:10, cursor:"pointer"}} title="Remover do Histórico">🗑️</button>
              )}
            </div>
...
/* ═══════ ADMIN PANEL ═══════ */
function AdminPanel({admins,setAdmins,ops,setOps,cl,setCl,pr,setPr,cfg,setCfg,campanhas,setCampanhas,setTela,setRole,adminSel,setAdminSel,opPrizes,setOpPrizes,adminLogs,setAdminLogs,logAdminAction,reverterAcao,checkM,customConfirm,customPrompt}){
    ...
    <div style={{background:"#fff",borderRadius:10,border:`1px solid ${expA?C.az:C.bd+"66"}`,overflow:"hidden"}}>
      ...
      {expA && <div style={{padding:12,borderTop:`1px solid ${C.bd}33`,background:"#fafafa"}}>
         ...
         <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
            {a.valida !== false && s!=="approved" && <button onClick={async()=>await updateStatusNative("approved")} style={{flex:1,minWidth:"35%",background:C.vd,color:"#fff",border:"none",borderRadius:10,padding:"10px 8px",fontSize:12,fontWeight:800,cursor:"pointer"}}>✅ Aprovar Autenticação</button>}
            {a.valida !== false && s!=="rejected" && <button onClick={async()=>await updateStatusNative("rejected")} style={{flex:1,minWidth:"35%",background:C.rd,color:"#fff",border:"none",borderRadius:10,padding:"10px 8px",fontSize:12,fontWeight:800,cursor:"pointer"}}>❌ Recusar Autenticação</button>}
            <button onClick={async()=>await excluirAuthNative()} style={{flex:1,minWidth:"35%",background:"#374151",color:"#fff",border:"none",borderRadius:10,padding:"10px 8px",fontSize:12,fontWeight:800,cursor:"pointer"}}>🗑️ Excluir Autenticação</button>
         </div>
      </div>}
    </div>
  );
...
function OpVoucherCard({p, cli, cfg, onClose}){
  ...
  return(<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.85)",zIndex:9999,display:"flex",alignItems:"center",justifyContent:"center",padding:10,backdropFilter:"blur(5px)"}} onClick={(e)=>{ if(e.target === e.currentTarget) onClose(); }}>
    ...
    <div style={{position:"fixed", left: "-9999px", top: 0}}>
      <div id="cupom-capture-admin" style={{background:"#fff", width: "600px", minHeight: "600px", height: "auto", fontFamily: "'Nunito', sans-serif", textAlign: "center", display: "block"}}>
